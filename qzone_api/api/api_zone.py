...
from ..utils import *
from ..utils.html_parser import parse_zone_data
...
class ApiZone(ApiBase):
    ...
    async def _get_zone(self, target_qq: int, g_tk: int, cookies: str,page:int=1,count:int=10,begintime:int=0) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            return None
    async def _get_zone_list(self, target_qq: int, g_tk: int, cookies: str,page:int=1,count:int=10,begintime:int=0) -> Optional[Dict[str, Any]]:
        """获取空间动态列表"""
        connect = await self._get_zone(target_qq, g_tk,cookies,page=page,count=count,begintime=begintime)
        if connect:
            return parse_zone_data(clean_escaped_html(connect))
        return None
