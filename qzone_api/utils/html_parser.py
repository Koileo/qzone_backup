...
def _extract_content_from_html(html: str) -> str:
    """从 PC 版 html 字段里抽 <div class="f-info"> 里的文字，失败就返回空字符串"""
    if not isinstance(html, str):
        return ""
    try:
        m = re.search(r'<div class="f-info">(.*?)</div>', html, flags=re.S)
        if not m:
            return ""
        text = m.group(1)
        # 去掉所有标签
...
def parse_zone_data(data) -> dict:
    """
    解析 PC 版空间说说数据：
    - 入参 data：形如 '{ "code":0, "subcode":0, ..., data:{main:{...}, data:[{...},...]}}' 的字符串
    - 返回结构：{"status":"ok","total":N,"data":[ {...}, ... ]}
      每条 feed 至少包含:
    ...
    try:
        ...
        if not isinstance(raw_list, list):
            raise TypeError(f"data['data'] 不是 list，而是 {type(raw_list)}")
